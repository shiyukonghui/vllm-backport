...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        if drop_eagle_block:
            ...
            # NOTE(port): the coordinator already hands Mamba the post-drop
            # candidate and retains its state exactly there
            # (``get_replay_boundaries``); dropping again lands one block
            # below the retained checkpoint, so under sparse retention a
            # resend may miss. Verify at model-run time (see port notes).
